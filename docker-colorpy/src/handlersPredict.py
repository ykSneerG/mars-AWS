...
from src.code.predict.interpolateTarget import (
    FilterMahalanobis,
    ModernRBFkernel,
    ModernCubicHermeticSplineInterpolator,
)
...
class Predict_SynlinV4Multi_Handler(BaseLambdaHandler):
    ...
    def handle(self):
        ...
        dcs_gradient = np.linspace(0, 100, STEPS)
        dcs = GradientMixGenerator.generate_dcs_multilin_tolist(
            dcs_gradient, len(corners)
        )
        ...
        rails_colors = trafo.Cs_SNM2MULTI(
            rails_flattened, {"SNM": True, "LCH": True, "HEX": True}
        )
        ...
        jd.update({"color": rails_colors, "elapsed": self.get_elapsed_time()})
        return self.get_common_response(jd)
...
class Predict_SynHyperFourV4_Parallel_Handler(BaseLambdaHandler):
    ...
    def optimal_chunksize(
        ...
        color_count: int,
        steps: int,
        max_color_count_per_chunk: int = 500,
        max_chunks: int = 50,
    ):
    ...
    def handle(self):
        ...
        towerA_flattened_elapsed, towerA_flattened = self.excute_lambda_rails(
            SLS_PARAMS, towerA_chunked, {"SNM": True}
        )

        towerB_chunked = self.chunk_rail_numpy(towerB, chunksize)
        towerB_flattened_elapsed, towerB_flattened = self.excute_lambda_rails(
            SLS_PARAMS, towerB_chunked, {"SNM": True}
        )
        ...
        rail_flattened_elapsed, rail_flattened = self.excute_lambda_rails(
            SLS_PARAMS, rail_chunked
        )
        ...
        btx_json_result = btx_json.store_S3(
            object_name, btx_json_content, f"data/{object_name}.json"
        )
        ...
        datastore_cgats_result = btx_cgats.store_S3(
            object_name, btx_cgats_content, f"data/{object_name}.txt"
        )
...
class Predict_SynHyperFiveV4_Parallel_Handler(BaseLambdaHandler):
    ...
    def handle(self):
        ...
        [0.0, 0.0, 0.0, 0.0, 100.0]         ----O
        [0.0, 0.0, 0.0, 100.0, 0.0]         ---K-
        [0.0, 0.0, 0.0, 100.0, 100.0]       ---KO
        [0.0, 0.0, 100.0, 0.0, 0.0]         --Y--
        [0.0, 0.0, 100.0, 0.0, 100.0]       --Y-O
        [0.0, 0.0, 100.0, 100.0, 0.0]       --YK-
        [0.0, 0.0, 100.0, 100.0, 100.0]     --YKO
        [0.0, 100.0, 0.0, 0.0, 0.0]         -M---
...
class InterpolatePairs(BaseLambdaHandler):
    ...
    def handle(self):
        ...
        try:
            jd.update(
                {"color": SlsHelper.process_colors_batch(SLS_PARAMS, rail, dst_types)}
            )
        except Exception as e:
...
class PredictCYNSN_Handler(BaseLambdaHandler):
    ...
    def handle(self):
        ...
        rbf_model.set_src_pcs(fit_pcs)
        rbf_model.set_smoothness(1e-2)
        rbf_model.set_degree(3)
...
class InterpolateTarget_modernRBF_Handler(BaseLambdaHandler):
    ...
    def handle(self):
        ...
        src_pcs = self.event["src_pcs"]
        dst_dcs = self.event["dst_dcs"]
        smooth = self.event.get(
            "smoothing", 1e-8
        )  # scientific notation for the number 0.00000001 (1 × 10⁻⁸).
        ...
        if interpol == 3:
            kernel = ModernRBFkernel.INVERS_MULTIQUADRATIC
        ...
        rbf_model.set_src_pcs(np.array(src_pcs))
        ...
        dst_pcs = rbf_model.interpolate(dst_dcs)
...
class InterpolateTarget_cubicHermiteSpline_Handler(BaseLambdaHandler):
    ...
    def handle(self):
        ...
        jd = {"elapsed": self.get_elapsed_time(), "result": result}
        return self.get_common_response(jd)
...
class InterpolateTarget_Handler(BaseLambdaHandler):
    ...
    def handle(self):
        ...
        if self.event.get("steps"):
            dst_dcs = GradientMixGenerator.generate_dcs_tolist(
                np.linspace(0, 100, self.event["steps"]), 4
            )
...
class InterpolateTarget_RBF_Handler(BaseLambdaHandler):
    ...
    def handle(self):
        ...
        jd.update(
            {"elapsed": self.get_elapsed_time(), "dst_dcs": dst_dcs, "dst_pcs": dst_pcs}
        )
        return self.get_common_response(jd)
